...
class Player:
    ...
    def play(self, df):
        ...
        self.max_area = int(self.width * self.height * 0.5)
        self.min_area = int(self.width * self.height * 0.01)
            ...
            if predict is True:
                ...
                if tracking_state is False and saliency_state is True:
                    ...
                    for elem in objects_in_frame:
                        ...
                        center_x = int((2 * x + w) / 2)
                        center_y = int((2 * y + h) / 2)
                        distance = math.sqrt(pow(current_x - center_x, 2) + pow(current_y - center_y, 2))
                        if distance < min_distance and self.is_area(current_x, current_y, x, y, w, h):
                ...
                # 有追踪：
                    ...
                    if tracker is not None:
                        ...
                        if ok is True:
                            ...
                            # 实际点在追踪区域内
                            if self.is_area(current_x, current_y, bbox[0], bbox[1], bbox[2], bbox[3]):
                                # 判断有没有更好的追踪
                                # 找到更小的显著性框
                                tracking_distance = math.sqrt(pow(current_x - int((2 * bbox[0] + bbox[2]) / 2), 2) + pow(current_y - int((2 * bbox[1] + bbox[3]) / 2), 2))
                                ...
                                for elem in objects_in_frame:
                                    (x, y, w, h) = elem
                                    # 如果显著性框在追踪内，且到实际点距离比追踪框更近
                                    if int(x) > int(bbox[0]) and int(x + w) < int(bbox[0] + bbox[2]) and int(y) > int(bbox[1]) and int(y + h) < int(bbox[1] + bbox[3]):
                                        saliency_distance = math.sqrt(pow(current_x - int((2 * x + w) / 2), 2) + pow(current_y - int((2 * y + h) / 2), 2))
                                ...
                                yaw, pitch = self.trans_sphere(min_x_center, min_y_center)
                                predict_coordinates = [next_frame_no, yaw, pitch]
                            # 实际点在追踪区域外，进行距离比较，如果实际点正在走向追踪器，继续追踪；如果实际点正在远离或者不定，取消追踪
                            else:
                                del tracker
                                tracking_state = False
