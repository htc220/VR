...
class SVRpolicy(Policy):
    ...
    def predict(self, train, next_x):
        ...
        train_yaw = sample[:, [0, 1]]
        train_pitch = sample[:, [0, 2]]
        next_yaw = Predict(train_yaw, next_x).SVR()
        next_pitch = Predict(train_pitch, next_x).SVR()
        next_yaw = (next_yaw + 180) % 360 - 180
        if next_pitch > 90:
            next_pitch = 90
        elif next_pitch < -90:
...
class Ridgepolicy(Policy):
    ...
    def predict(self, train, next_x):
        ...
        train_yaw = sample[:, [0, 1]]
        train_pitch = sample[:, [0, 2]]
        next_yaw = Predict(train_yaw, next_x).Ridge()
        next_pitch = Predict(train_pitch, next_x).Ridge()
        next_yaw = (next_yaw + 180) % 360 - 180
        if next_pitch > 90:
            next_pitch = 90
        elif next_pitch < -90:
...
class RandomForestpolicy(Policy):
    ...
    def predict(self, train, next_x):
        ...
        train_yaw = sample[:, [0, 1]]
        train_pitch = sample[:, [0, 2]]
        next_yaw = Predict(train_yaw, next_x).RandomForest()
        next_pitch = Predict(train_pitch, next_x).RandomForest()
        next_yaw = (next_yaw + 180) % 360 - 180
        if next_pitch > 90:
            next_pitch = 90
        elif next_pitch < -90:
...
class MCswitchpolicy(Policy):
    def __init__(self):
        ...
        self.video_name = self.video.video_name
        self.predict_time = self.configure.predict_time
        self.policys = [LSRpolicy(), TLPpolicy(), Ridgepolicy()]
        np_name = self.video_name + '_' + str(self.predict_time) + 's.npy'
        np_path = os.path.join('markovchain', np_name)
        if os.path.exists(np_path):
            self.markovchain = np.load(np_path)
...
class KLRpolicy(Policy):
    ...
    def predict(self, train, next_x):
        ...
        train_yaw = sample[:, [0, 1]]
        train_pitch = sample[:, [0, 2]]
        next_yaw = Predict(train_yaw, next_x).KLR()
        next_pitch = Predict(train_pitch, next_x).KLR()
        result = [next_x, next_yaw, next_pitch]
        return result

class KTLPpolicy(Policy):
    def __init__(self):
        self.name = "KLSR"
    ...
    def predict(self, train, next_x):
        ...
        train_yaw = sample[:, [0, 1]]
        train_pitch = sample[:, [0, 2]]
        next_yaw = Predict(train_yaw, next_x).KTLP()
        next_pitch = Predict(train_pitch, next_x).KTLP()
        result = [next_x, next_yaw, next_pitch]
        return result
